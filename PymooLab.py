...
class PrimaryWorkflowTabBar(QTabBar):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._brand_pixmap = self._trim_transparent_padding(
            QPixmap(str(Path(__file__).resolve().parent / "pymoolab.png"))
        )
    ...
    def paintEvent(self, event: object) -> None:
        ...
        painter.fillRect(self.rect(), QColor(AppStyles.PANEL))
        painter.setPen(QPen(QColor(AppStyles.BORDER_LIGHT), 1))
        ...
        for index in range(self.count()):
            ...
            if selected:
                selected_rect = tab_rect.adjusted(8, 8, -8, -8)
                painter.setPen(Qt.PenStyle.NoPen)
                painter.setBrush(QColor("#E5F2FF"))
                ...
                painter.setBrush(QColor(AppStyles.PRIMARY))
            ...
            painter.setPen(
                QColor(AppStyles.PRIMARY if selected and enabled else AppStyles.TEXT_MUTED if enabled else AppStyles.TEXT_DISABLED)
            )
...
def _navigation_icon(name: str) -> QIcon:
    ...
    icon.set_color(QColor(AppStyles.TEXT_MUTED), QIcon.Mode.Normal, QIcon.State.Off)
    icon.set_color(QColor(AppStyles.PRIMARY), QIcon.Mode.Normal, QIcon.State.On)
    icon.set_color(QColor(AppStyles.TEXT_DISABLED), QIcon.Mode.Disabled, QIcon.State.Off)
    return icon
...
def _install_legacy_runtime_aliases() -> None:
    ...
    # Prefer the real backend-aware modules (MLX/JAX) when importable. This MUST
    # run before the _ensure_virtual_package calls below, otherwise a virtual
    # "util"/"core" package shadows the real packages and the legacy CPU-only
    # shims win, silently forcing the effective backend to numpy (disabling MLX
    # and JAX even when available). The shims remain a fallback for environments
    # where these real packages cannot be imported.
    for _real_module in ("util.array_backend", "core.algorithm"):
...
class PymooExperimentWindow(QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("PymooLab - An Open-Source Visual Analytics Framework for Multi-Objective Optimization")
    ...
    def _setup_menu(self) -> None:
        ...
        act_about = QAction("About PymooLab", self)
    ...
    def _show_about_dialog(self) -> None:
        ...
        QMessageBox.about(
            ...
            "About PymooLab",
            (
                f"<b>PymooLab</b> — release {_app_version}<br>"
                ...
                "If PymooLab contributes to your published work, please cite the "
                "arXiv preprint listed in the README "
                "(<i>PymooLab: An Open-Source Visual Analytics Framework for "
    ...
    def _setup_ui(self) -> None:
        ...
        self.tabs.setAccessibleName("PymooLab primary workflow tabs")
        ...
        # Persistent, unobtrusive attribution with a clickable research-group
        # link; lives in the status bar so it never competes with the workflow.
        metis_link = QLabel(
            '<a href="https://metisbr.com" style="text-decoration: none;">'
            "METISBr research group &mdash; metisbr.com</a>"
        )
        metis_link.setOpenExternalLinks(True)
        metis_link.setToolTip(
            "Multi-Objective and Many-Objective Optimization Research (UFOP, Brazil). "
            "Opens metisbr.com in your browser."
        )
        self.statusBar().addPermanentWidget(metis_link)
    ...
    def _build_llm_tab(self) -> QWidget:
        ...
        self.llm_api_key_edit.setAccessibleName("Anthropic API key")
        self.llm_api_key_edit.setAccessibleDescription("Enter the API key used by the LLM Assistant. The value is hidden while you type.")
        ...
        self.llm_problem_scope_summary_label = QLabel(
            "Describe the problem in natural language. PymooLab refines the prompt for pymoo/PymooLab compatibility."
        )
        ...
        self.llm_save_key_btn.setStyleSheet(AppStyles.secondary_button_style())
        ...
        self.llm_load_key_btn.setStyleSheet(AppStyles.secondary_button_style())
        ...
        self.llm_get_key_btn.setStyleSheet(AppStyles.secondary_button_style())
        ...
        self.llm_validate_key_btn.setStyleSheet(AppStyles.secondary_button_style())
    ...
    def _llm_problem_prompt_policy_summary_text(self) -> str:
        ...
        return (
            ...
            "Write the request in natural language; PymooLab refines the prompt for pymoo/PymooLab compatibility."
        )
    ...
    def _refresh_llm_metric_mode_summary(self) -> None:
        ...
        self.llm_metric_mode_summary_label.setText(
            "Metric generation uses Claude/Anthropic web_search (GitHub-only) to recover or convert trustworthy implementations, "
    ...
    def _llm_hint_text(self) -> str:
        ...
        if token == "metric":
            return (
                "Generate a local metric plugin and the matching _JAX variant (create_metric(context)) with Claude "
                "(Anthropic Messages API, model claude-sonnet-4-6). The workflow validates both modules before saving to metrics/. "
                "Use precise names when possible (HV/IGD/GD/DeltaP), and describe semantics and inputs explicitly. "
                "Scope: one plugin per request; benchmark surveys and literature extraction belong outside this mode."
            )
        if token == "benchmark_survey":
            return (
                "Run a benchmark survey extraction with Claude (Anthropic Messages API) and web_search "
                "to build a source-linked inventory of benchmark functions and reported dimensions. "
                "The output is an evidence-backed report with limitations, not a plugin module. "
                "Then use 'Survey -> Problem Plugin' to select one entry and generate a wrapper."
            )
        return (
            "Generate one pymoo Problem plugin (CPU + _JAX) from natural language using Claude "
            "(Anthropic Messages API, model claude-sonnet-4-6), with code aligned to PymooLab conventions. "
    ...
    def _build_results_tab(self) -> QWidget:
        ...
        self.stats_header_label = QLabel("PymooLab | Statistical View: Detailed Runs")
    ...
    def _refresh_results_table(self) -> None:
        ...
        self._last_stat_result = None
        if hasattr(self, "stats_header_label"):
            self.stats_header_label.setText("PymooLab | Statistical View: Detailed Runs")
