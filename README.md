...
| Item | Value |
|---|---|
| PymooLab release | `1.0.7` |
| Catalog snapshot | `2026-07-02` |
...
├──────────────────────┬───────────────────────────────────────────┤
│   core/ (base)       │   core/ (modular logic)                  │
│   ├── algorithm.py   │   ├── analysis/   └── stat_tests         │
│   ├── problem.py     │   ├── execution/  ├── backend            │
│   ├── population.py  │   │               └── reprod.            │
│   ├── individual.py  │   ├── llm/        └── formulation        │
│   ├── mating.py      │   ├── mcdm/       └── decision           │
│   ├── parameters.py  │   └── registry/   ├── backend            │
│   └── duplicate.py   │                   ├── plugin_dirs        │
│                      │                   └── rollout            │
├──────────────────────┴───────────────────────────────────────────┤
│   util/  ├── array_backend.py  ├── display/  ├── nds/            │
│          ├── ref_dirs.py       └── optimum.py                   │
├──────────────────────────────────────────────────────────────────┤
...
├── version.py                   # Package semantic version (__version__ = 1.0.2)
├── app.png                      # Application header image
├── pymoolab.png                 # Application logo
│
├── core/                        # Unified core: base classes + modular logic
│   ├── __init__.py
...
│   │   ├── qt_bridges.py        # QObject bridge for async LLM tasks
...
│   └── registry/
│       ├── specs.py             # AlgorithmSpec/ProblemSpec/MetricSpec/OperatorSpec
...
│   │   ├── __init__.py
│   │   └── ssw.py
│   ├── dnv_maoa/                # DNV-MaOEA: dynamic normal vectors + EEJ
│   │   ├── __init__.py
│   │   └── dnv_maoa.py
│   ├── ssw/                     # Example: SSW algorithm
│   │   ├── __init__.py
...
`ABSAEA`, `ACMMEA`, `AdaW`, `ADSAPSO`, `AENSGAII`, `AFSEA`, `AGEII`, `AGSEA`, `AMGPSL`, `ANSGAIII`, `APSEA`, `ARMOEA`, `AVGSAEA`, `BCEIBEA`, `BCEMOEAD`, `BiCo`, `BiGE`, `BLEAQII`, `BLSAEA`, `C3M`, `CAEAD`, `CAMOEA`, `CCGDE3`, `CCMO`, `cDPEA`, `CGLP`, `CLIA`, `CMaDPPs`, `CMDEIPCM`, `CMEGL`, `CMME`, `CMMO`, `CMOBR`, `CMOCSO`, `CMODEFTR`, `CMODRL`, `CMOEA_CD`, `CMOEA_GBSS`, `CMOEAD`, `CMOEAMS`, `CMOEAMSG`, `CMOEBOD`, `CMOEMT`, `CMOES`, `CMOQLMT`, `CMOSMA`, `CNSDEDVC`, `CoMMEA`, `CPSMOEA`, `CSEA`, `CSEMT`, `CTSEA`, `DAEA`, `DBEMTO`, `DCNSGAIII`, `DEAGNG`, `DGEA`, `DirHVEI`, `DISK`, `DISK_2025`, `DISKplus`, `DKCA`, `DMMOEA`, `DMOEAeC`, `dMOPSO`, `DNNSGAII`, `DPCPRA`, `DPPPS`, `DPVAPS`, `DRLOSEMCMO`, `DRLSAEA`, `DSPCMDE`, `DSSEA`, `DVCEA`, `DWU`, `EAGMOEAD`, `EDNARMOEA`, `EFRRR`, `EIMEGO`, `EM_SAEA`, `EMCMMS`, `EMCMO`, `EMMOEA`, `eMOEA`, `EMOSKT`, `EMyOC`, `ENSMOEAD`, `ESBCEO`, `FDV`, `FLEA`, `FRCGM`, `GCNMOEA`, `GCSMaOEA`, `GDE3`, `GFMMOEA`, `GLMO`, `gNSGAII`, `GPSOM`, `GrEA`, `GWASFGA`, `HEA`, `HeEMOEA`, `HHCMMEA`, `hpaEA`, `HREA`, `HypE`, `IBEA`, `ICMA`, `IDBEA`, `IMCMOEAD`, `IMMOEA`, `IMMOEAD`, `IMTCMO`, `IMTCMO_BS`, `ISIBEA`, `Izui`, `KLEA`, `KLNSGAII`, `KnEA`, `KRVEA`, `KTA2`, `KTS`, `LARC_NSGA3`, `LCMEA`, `LCSA`, `LDSAF`, `LERD`, `LMEA`, `LMOCSO`, `LMOEADS`, `LMPFE`, `LRMOEA`, `LSMOF`, `MaOEACSS`, `MaOEADDFC`, `MaOEAIGD`, `MaOEAIT`, `MaOEARD`, `MCCMO`, `MCEAD`, `MFFS`, `MFOSPEA2`, `MGCEA`, `MGSAEA`, `MMEAPSL`, `MMEAWI`, `MMOPSO`, `MO_Ring_PSO_SCD`, `MOBCA`, `MOCell`, `MOCGDE`, `MOCMA`, `MOEACKF`, `MOEAD2WA`, `MOEADAWA`, `MOEADCMA`, `MOEADCMT`, `MOEADD`, `MOEADDAE`, `MOEADDCWV`, `MOEADDE`, `MOEADDQN`, `MOEADDRA`, `MOEADDU`, `MOEADDYTS`, `MOEADEGO`, `MOEADFRRMAB`, `MOEADM2M`, `MOEADMRDL`, `MOEADPaS`, `MOEADPFE`, `MOEADSTM`, `MOEADUR`, `MOEADURAW`, `MOEADVA`, `MOEADVOV`, `MOEAIGDNS`, `MOEANZD`, `MOEAPC`, `MOEAPSL`, `MOEARE`, `MOEGS`, `MOL2SMEA`, `MOMBIII`, `MOMFEA`, `MOMFEAII`, `MOMFEASADE`, `MONAS`, `MOPSO`, `MOPSO_CD`, `MOSD`, `MPAES`, `MPMMEA`, `MPSOD`, `MSCEA`, `MSCMO`, `MSEA`, `MSKEA`, `MSOPSII`, `MTCMO`, `MTDEMKTA`, `MTEADDN`, `MTS`, `MultiObjectiveEGO`, `MyODEMR`, `NBLEA`, `NMPSO`, `NNDREAMO`, `NNIA`, `NRVMOEA`, `NSBiDiCo`, `NSGAIIARSBX`, `NSGAIIconflict`, `NSGAIIDTI`, `NSGAIIIEHVI`, `NSGAIISDR`, `NSGA3Local`, `NSLS`, `NUCEA`, `onebyoneEA`, `OSPNSDE`, `ParEGO`, `PBNSGAIII`, `PBRVEA`, `PCSAEA`, `PEA`, `PEAplus`, `PeEA`, `PESAII`, `PICEAg`, `PIEA`, `PIMD`, `PMMOEA`, `POCEA`, `PPS`, `PRDH`, `PREA`, `REMO`, `RGA_M1_2`, `RGA_M2_2`, `RMMEDA`, `RMOEADVA`, `RPDNSGAII`, `RPEA`, `RSEA`, `RVEAa`, `RVEAiGNG`, `S3CMAES`, `SAMOEA_TL2M`, `SCDAS`, `SCEA`, `SECSO`, `SFADE`, `SGEA`, `SGECF`, `SIBEA`, `SIBEAkEMOSS`, `SLMEA`, `SMEA`, `SMOA`, `SMPSO`, `SMSEGO`, `SNSGAII`, `SparseEA`, `SparseEA2`, `SPEA2SDE`, `SPEAR`, `SRA`, `SSCEA`, `SSDE`, `SSW`, `DNV_MaOEA`, `SSW_RDPA`, `SVRNSGAII`, `tDEA`, `tDEACPBI`, `TEA`, `TELSO`, `TiGE2`, `ToP`, `TPCMaO`, `TriMOEATAR`, `TSNSGAII`, `TSSparseEA`, `TSTI`, `Two_Arch2`, `URCMO`, `VaEA`, `WASFGA`, `WOF`, `WVMOEAP`

> Plus native pymoo algorithms: NSGA-II, NSGA-III, U-NSGA-III, RVEA, MOEA/D, AGE-MOEA, AGE-MOEA-II, C-TAEA, SMS-EMOA, SPEA2, etc.

Catalog notes (2026-07-02):

- `GCSMaOEA` (alias `GCS-MaOEA`) is the METISBr Geometry-Coupled Search algorithm for many-objective problems with irregular Pareto fronts: intrinsic-dimension-adaptive reference vectors (PCA-energy + TwoNN committee, piecewise per-component generation), direction-aware decision-variable analysis with budgeted linkage learning, learned convergence targets, and a dynamic three-archive engine.
- `DEAGNG` (alias `DEA-GNG`), `RVEAiGNG` (alias `RVEA-iGNG`), `ANSGAIII` (alias `A-NSGA-III`), and `AdaW` are now complete native ports of their PlatEMO reference implementations (previously thin delegating stubs).
...
- Accelerated metric variants suffixed `_MLX` (Apple Silicon) and `_JAX` are generated automatically from the CPU registry and appear alongside the base names when the corresponding backend is available.
