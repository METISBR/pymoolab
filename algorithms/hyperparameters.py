...
from util.array_backend import get_array_module
from core.parameters import get_params, flatten, set_params, hierarchical
...
def stats_single_objective_mean(rets):
    ...
    xp = get_array_module(F[0] if F else None)
    F, G, H = xp.array(F), xp.array(G), xp.array(H)
...
def stats_avg_nevals(rets):
    xp = get_array_module()
    return dict(F=xp.array([ret.algorithm.evaluator.n_eval for ret in rets]).mean())
